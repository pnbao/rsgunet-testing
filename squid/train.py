...
def log_hook(sess, log_fetches):
    ...
    log.info('Step {} | loss = {:.4f}| loss_content = {:.4f} | loss_texture = {:.4f} | '
             'loss_color = {:.4f} | loss_tv = {:.6f} |loss_Mssim = {:.4f} |discim_accuracy =  {:.4f} | psnr = {:.2f} dB|ssim = {:.4f}'.format(
        step,
...
def main(args, data_params):
    ...
    with tf.name_scope('optimizer'):
        ...
        with tf.control_dependencies([updates]):
            ...
            minimize_discrim = opt.minimize(-loss_texture, name='discriminator', global_step=global_step,
    ...
    with tf.name_scope("moving_averages"):
        ...
        update_ma = ema.apply(
            [loss, loss_content, loss_texture, loss_color, loss_Mssim, loss_tv, discim_accuracy, psnr, loss_ssim])
...
if __name__ == '__main__':
    ...
    train_grp.add_argument('--learning_rate', default=5e-4, type=float,
    ...
    data_grp.add_argument('--data_pipeline', default='ImageFilesDataPipeline',
                          help='classname of the data pipeline to use.', choices=dp.__all__)
    data_grp.add_argument('--output_resolution', default=[100, 100], type=int, nargs=2,
